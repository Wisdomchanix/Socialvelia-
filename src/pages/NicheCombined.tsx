...
const NicheCombined: React.FC = () => {
  ...
  const generateSuggestions = (answers: string[]) => {
    setLoading(true);
      ...
      <div className="w-full max-w-5xl xl:max-w-6xl relative">
        ...
        <div className="bg-[#0A0219]/80 p-6 md:p-10 rounded-2xl border border-white/10 shadow-2xl backdrop-blur-md">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-lg md:text-2xl font-bold">Find your perfect niche</h1>
              <p className="text-xs md:text-sm text-gray-400 mt-1 max-w-xl">
                Answer a few quick questions and we’ll suggest tailored content niches and ideas.
              </p>
            </div>
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="md:flex-1">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <h2 className="font-semibold">{questions[currentIndex].question}</h2>
                      ...
                      >
                        <div>
                          ...
                          <div className="text-xs text-gray-400 mt-1">
                            This only runs locally — replace with your Gemini API later.
                          </div>
                        </div>
                    ...
                    {suggestions.map((sug, i) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <h3 className="text-lg font-semibold">{sug.name}</h3>
                            <p className="text-xs text-gray-300 mt-1">{sug.reason}</p>
                          </div>
                          <div className="flex flex-col items-end gap-2">
                            <button
                              onClick={() =>
                                copyToClipboard(`${sug.name}\n\n${sug.reason}`)
                              }
                        ...
                        <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                          <div className="bg-[#070812] p-3 rounded-lg">
                            ...
                            <ul className="mt-2 text-sm text-gray-200 space-y-1">
                              {sug.trends.map((t, j) => <li key={j}>• {t}</li>)}
                            </ul>
                          </div>
                          <div className="bg-[#070812] p-3 rounded-lg">
                            <p className="text-xs text-gray-400">Audience</p>
                            <ul className="mt-2 text-sm text-gray-200 space-y-1">
                              {sug.audience.map((a, j) => <li key={j}>• {a}</li>)}
                            </ul>
                          </div>
                          <div className="bg-[#070812] p-3 rounded-lg">
                            <p className="text-xs text-gray-400">Content Ideas</p>
                            <ul className="mt-2 text-sm text-gray-200 space-y-1">
                              {sug.ideas.map((i2, j) => <li key={j}>• {i2}</li>)}
                            </ul>
                          </div>
                        </div>
                      </motion.div>
