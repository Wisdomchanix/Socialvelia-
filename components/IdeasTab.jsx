...
const IdeasTab = () => {
  ...
  const [hasGenerated, setHasGenerated] = useState(false); // NEW: Track if AI already generated
  ...
  const handleAnswer = (key, value) => {
    ...
    // Only auto-advance for option questions, not for the final submit
    if (currentStep < questions.length - 1) {
      setTimeout(() => {
        setCurrentStep((prev) => prev + 1);
      }, 400);
    }
    // REMOVED: Auto-generate on last question answer
  };
  ...
  const generateIdeasFromAI = async () => {
    ...
    if (hasGenerated) {
      console.log("Ideas already generated, skipping...");
      return;
    }

    setLoadingAI(true);
    try {
      console.log("Generating AI ideas with answers:", answers);
      ...
      // FIX: Use the correct property names from your API
      if (data.payload?.ideas && Array.isArray(data.payload.ideas)) {
        setIdeas(
          data.payload.ideas.map((idea) => ({
            title: idea.title || "Untitled Idea",
            desc: idea.description || idea.desc || "No description available",
          }))
        );
      } else {
        console.warn("AI responded without ideas in expected format:", data);
      ...
      <div className="max-w-5xl mx-auto text-center">
        ...
        {loadingAI ? (
          <div className="py-8 text-gray-400">
            Generating personalized ideas for you…
          </div>
        ) : (
            ...
            >
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  ...
                  } py-3 bg-gradient-to-r from-[#F1824A] to-[#9DC88D] rounded-xl text-black font-semibold hover:opacity-90 transition disabled:opacity-50`}
                >
                  {loadingAI
                    ? "Generating..."
